...
var BlockGenerator = function () {
  ...
  _createClass(BlockGenerator, [{
    ...
    value: function processInlineElement(element) {
      ...
      if (styleAttribute) {
        (function () {
          var customCssMapToStyle = {};
          var normalizeStyle = function normalizeStyle(str) {
            return str.replace(/ /g, '').replace(/;/g, '');
          };

          // Convert react styles to css string values
          Object.keys(_this.customStyleMap).forEach(function (key) {
            customCssMapToStyle[normalizeStyle((0, _toStyle.string)(_this.customStyleMap[key]))] = key;
          });
          ...
          try {
            for (var _iterator = styleAttribute.split(';')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              var styleValue = _step.value;

              var styleAttr = normalizeStyle(styleValue);
              if (styleAttr) {
...
function addStyleFromStyleAttribute(styleSet, styleAttributeValue, customCssMapToStyle) {
